...
**Snappa** is a window management tool for GNOME that allows users to snap windows to layouts.

Snappa's core philosophy is flexibility: users can freely define their own [Spaces](../space/), [Layout Groups](../layout-group/), and [Layouts](../layout/) to match their workflow. Instead of forcing a fixed set of window arrangements, Snappa provides a framework for users to build their ideal window management system.
...
- **Multi-Monitor Support**: Each monitor can have different layout options, and Snappa remembers configurations for different [Monitor Environments](../monitor-environment/)

## How to Trigger

Snappa can be triggered in multiple ways:
...
- See [Main Panel](../main-panel/) for the UI that appears when Snappa is triggered
